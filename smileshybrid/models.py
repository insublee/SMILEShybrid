...
class SMILES_hybrid(AbstractModel):
    """
    이거 수정하시면 됩니다~~ __init__이랑 forward만 원하는대로 바꾸시면 되용.
    """
    def __init__(self, config:DictConfig) -> None:
        ...
        #self.mlp = MLP(config.data.n_sdf_features + config.data.n_pfp_features + config.data.n_mordred_features, self.transformer.model.config.hidden_size)
        self.mlp = MLP(config.data.n_sdf_features + config.data.n_pfp_features, self.transformer.model.config.hidden_size)
    ...
    def forward(self, **inputs):
        for key in inputs.keys():
            assert ~torch.isnan(inputs[key]).any(), f'{key} has nan!!!'
        
        cnn_out = self.cnn(inputs['img'])
        #mlp_in = torch.cat([inputs['sdf_features'], inputs['pfp_features'], inputs['mordred_features']], dim=1)
        mlp_in = torch.cat([inputs['sdf_features'], inputs['pfp_features']], dim=1)
        ...
        logits  = self.transformer(input_ids=inputs['input_ids'],
